...
module.exports = function (animations) {
  ...
  const api = {
    ...
    set targetOffset (o) {
      targetOffset = Math.max(0, Math.min(o, (animations.length - 1) * stratum.height))
    },
  ...
  animations = animations.map((name, i) => {
    let Animation = require(path.join(paths.animations, name))
    return new Animation(api, i * stratum.height)
  })
